...
WHY: Das Killer-Feature des Exploit-Modus (docs/doctrine/TRAINER_DESIGN.md §2) — die live gelernten
...
def _observation(model) -> tuple[str, str] | None:
    ...
    if f is not None and f > READ_OVERFOLD:
        return ("overfold", f"you fold to bets a lot ({_pct(f)}% at n={model.faced_bet}) "
                            "— so it bluffs you more often.")
    if f is not None and f < READ_STATION:
        return ("station", f"you almost never fold to bets ({_pct(f)}% at n={model.faced_bet}) "
                           "— so it has nearly stopped bluffing you and only bets for value.")
    ag = _stat(model, "aggression")
    if ag is not None and ag > READ_AGGRO_HI:
        return ("aggro_bluffcatch", f"you bet and raise very often ({_pct(ag)}% at "
                                    f"n={model.agg_opp}) — so it calls you down wider.")
    if ag is not None and ag < READ_AGGRO_LO:
        return ("passive_valueonly", f"you bet almost only with strong hands ({_pct(ag)}% at "
                                     f"n={model.agg_opp}) — so it believes your bets and folds more.")
    tb = _stat(model, "threebet")
    if tb is not None and tb > READ_THREEBET_HAPPY:
        return ("threebet_happy", f"you 3-bet strikingly often ({_pct(tb)}% at n={model.tb_opp}) "
                                  "— so it keeps playing wider against your 3-bets.")
    return None
...
def _progress(model) -> str:
    ...
    return f"not enough data yet (bets you faced: {min(faced, MIN_FACED_BET)}/{MIN_FACED_BET})."
    ...
    for seat in sorted(bots):
        ...
        if obs is None:
            fallback.append({"seat": seat, "name": name,
                             "beobachtung_de": _markup(f"Seat {seat} ({name}): {_progress(model)}")})
            continue
        trigger, satz = obs
        ...
        entries.append({"seat": seat, "name": name,
                        "beobachtung_de": _markup(f"Seat {seat} ({name}) noticed: {satz}")})
...
def _selftest():
    ...
    assert any("fold to bets a lot" in t and "75" in t and "n=12" in t for t in texts), texts
    assert any("raise very often" in t and "80" in t for t in texts), texts
    assert any("3-bet" in t and "strikingly often" in t and "25" in t for t in texts), texts
    # Konvergenz-Dedupe: der Overfold-Read erscheint genau EINMAL (Sitz 1, nicht auch Sitz 2)
    assert sum("fold to bets a lot" in t for t in texts) == 1
    ...
    assert len(gated) == 2 and all("not enough data yet" in e["beobachtung_de"] for e in gated)
    ...
    assert any("almost never fold to bets" in t and "17" in t for t in lt), lt   # 'bluffing' traegt einen Glossar-Span
    assert any("strong hands" in t and "10" in t for t in lt), lt
    ...
    print("OK - Panel rendert 2-4 englische Beobachtungen aus echten OppModel-Zaehlern, "
