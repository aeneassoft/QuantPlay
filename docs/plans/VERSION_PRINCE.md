...
> Ledger: audit `data/audit/audit_result.json` · research `../reports/RESEARCH_SWEEP_2026-07-05.md` · error budget
> `data/gtow_grades/error_prognosis.json`. After the ladder: the v4 decision (depth-limited CFR + neural leaves).


**The name is the thesis:** the cross-coaching replay (bot vs a human line, 2,332 decisions) said the perfect player
of that sample plays *the bot's discipline + a human's pressure*. This version fuses them. **Doctrine: ONLY bb count**
(GTO-score is dead as a target — measured: chasing it COST EV). Every ingredient below carries its evidence status;
nothing ships on hope. Full evidence trail: `docs/STATE.md` #4/#5, `../reports/GTOW_DOSSIER.md`, `data/gtow_grades/LEAK_MAP.md`.
...
**P(K|check) 9.2→17.3% ≈ the 18.2% prior — the check is now uninformative (an observer's read is dead)**; fold-to-stab
...
**C1 — River value/pressure uplift (the biggest bleed: −8.6 bb/100).** A human overbet line (thin max-value,
...
**C2 — The line planner (the "it doesn't really play poker" fix).** Minimal viable version: ONE plan enum chosen at
...
- **Q5 (hours): the product plays the WRONG config** (audit #3) — the HU/6-max apps run exploit-primary HEAD without
  the mode; human leak-hunting runs against the un-fixed bot. Launcher/env wiring once PRINCE passes mechanics.
...
- C1 (river discipline) is UPGRADED by audit #4: the spec (`../consults/river_corset_consult.md` Guards 1–3) + 66
...
- **Books-wave intake (landed):**
  ...
  - **B2 (1–2d, highest-confidence content fix): river value gate = equity vs the CALLING range (`POKERB_RIVER_ECALL`)**
    ...
    pp.86-87). The only lever that makes the +98bb human overbet-jam a GTO-legal line.
