...
> **About this document.** Verbatim transcript of a two-stage consultation (June 2026, GLM-Z1-9B brain phase; committed 2026-07-05): an OpenAI model (not named in the transcript) was asked for the river mathematics behind a "corset" — deterministic guards between the LLM brain's proposal and the engine action — after the brain measured −94.14 bb/100 over 100 hands vs GTO Wizard with the worst 10 hands (mostly river over-calls of huge bets) making up ~103 % of the loss. The planned second stage ("Claude: technical implementation") is an empty heading; the implementation went into `pokerbot/strategy/postflop_corset.py`.
> **English transcript, kept verbatim; what was adopted is recorded in docs/STATE.md and ../catalogs/MEASUREMENT_CATALOG.md.**
> Main conclusions:
> - Required equity to call a bet B into pot P is E_req = B/(P+3B); MDF = P/(P+B); the indifference bluff:value ratio is B:(P+2B) for both sides.
> - The corset's primary anchor is the local EV guard (never call with equity vs the betting range below B/(P+3B), margin 0.01–0.02); MDF is only a lower bound on total defense and must not be used to force extra calls vs a GTO opponent; for overbets (B ≥ 1.5P) cap the calling frequency at MDF + 0.1.
> - "Bet enough that they fold" is wrong on the river: choose size and bluff:value composition to maximize range EV; an all-in is evaluated purely by EV_shove = q·P + (1−q)·[E_hero·(P+2B) − (1−E_hero)·B], no dramatic weight.
