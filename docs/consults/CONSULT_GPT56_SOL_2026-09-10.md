...
> **About this document.** Verbatim transcript of the answer of OpenAI's `gpt-5.6-sol` (2026-09-10, reasoning effort high; see ../catalogs/CANDIDATES.md) to a 37,655-character briefing with our own fact check. Three questions: (1) the "Zwitter" — a hybrid bot that picks the supposedly best module per decision, (2) how to improve the flagship version, (3) the potential of v10.1 (H0 → H1). The briefing itself is not included; the model's text starts with its three corrections of the briefing.
> **German transcript; the conclusions that were adopted are recorded in English in docs/STATE.md and ../catalogs/MEASUREMENT_CATALOG.md.**
> Main conclusions:
> - Corrections up front: the GTOW competition and the main HU channel are 200 bb, not 100 bb (the Kaggle channel measures a different bot); −21.12 bb/100 is the anchor of v4 on PRINCE, not of the champion v5; the "missing 8 bb/100" and the asymptotic LCB limit −14.8 (about 6.3 bb/100 away) are the same leaderboard rule at different volumes.
> - The hybrid verdict: an executable containing all variants — yes; a per-decision module picker — no, a game-theoretic fallacy. Viable only as v5 baseline + exactly one coherent river expert + full v5 continuation + a conservative public selector (river entry, pot ≥ 15 bb, full support, p99 within the time budget), never falling back to the bare `bot.py`. Expected gain of the river hybrid: 0 to +3 bb/100 over v5; v9's +3.6 to +5.4 was a pre-registered hypothesis, not a prior.
> - Measurement first: a GTOW anchor of the exact productive v5 with a fingerprint; decompose ΔZ = trigger frequency × (ΔE − R_sel − L_seam); channels K3 exact best response, paired mirror, root bench, GTOW only for complete packages. Top 5 needs μ > −14.8: at −13 about 54,000 hands, at −11 about 12,200; development target −11 to −12.
> - The three things to do first: anchor v5, repair the river path (`river_gpu_guard`) and run the "GPU replaces TexasSolver river" test, test K2 against v5 on oracle ranges before writing any H0/H1 code. ICM, 6-max, the LLM brain, refuted exploit overlays and unclosed GPU action pickers do not belong in the HU cash policy.
