...
## ★★ MTT-SIM: 13,016-chip deficit autopsied (pre-fix artefact) + the determinism trap was SET ITERATION, not RNG (2026-08-04)
- **[measured, closed] The 13,016-chip deficit (1/91 tournaments, seed 500018, arm=druck, field=freq) is not
  reproducible in the COMMITTED code and is with high probability an artefact of the working state BEFORE
  747369f** (the 3 engine fixes — ante street/HU blinds/orphaned side-pot layer — were committed at 19:46;
  the instrumented 91-tournament run ran mid-development before that). Burden of proof: ~325 full tournaments + 235k
  hands chip-exact in detail (P(0 hits | rate 1/91) ≈ 3%); BOTH suspects refuted — guard=300 practically
  unreachable (longest hand 31 actions; the tail dies ~3 decades per +10) and the all-in-from-posts state is
  resolved chip-preservingly by `start_hand` via `_close_round`. The only destruction mechanism would be an
  unresolved loop exit → TRIPWIRE in `_play_hand`: annulment (refund) instead of pot destruction +
  chip conservation PER HAND with a full dump (audit=True → hard abort). **Culprit (measured fit): a
  destroyed ORPHANED SIDE-POT LAYER** — instrumented, they occur in ~1/30 tournaments (5 in 150:
  267/1,451/1,545/4,796/26,144 chips); 13,016 lies in the middle of the size distribution, the rarity matches the
  1/91 observation. Exactly these layers are refunded today by the 747369f fix (showdown refunds).
- **[measured, fixed] Cross-process divergence (same seed → finishing place 378/12/4) = PYTHONHASHSEED-dependent
  SET ITERATION, NOT the global random module** (measured: 0 module calls in the run; the only OS entropy =
  the immediately replaced `SixMaxBot.__init__` rng). Chain: `ps.range_top()` → set[str] → `list()` in
  `sixmax._decide` → MC combo order → the same rng draws hit different combos. Fix: `sorted()` at the
  chokepoint `equity_vs_class_range` (mirror of the 2026-07-05 fix in `ranges.combos_for_classes`);
  verified byte-identical across processes (full place+stack fingerprint, 2×2 runs). **3rd find of this
  trap in the repo — on paired divergence ALWAYS check PYTHONHASHSEED first (2 runs with the same hash seed),
  only then hunt the RNG.** Within ONE process the pairing chipEV↔druck was always intact (same salt).

## ★ TRAINER: the DU line should recognise deliberate slowplay traps (user, 2026-08-02 — trainer TEXT ONLY, NOT the HU GTO bot)
- **[deferred] `pokerbot/coach/range_story.py::_du_line`**: with a passive line + a monster the DU line says "more
  hand than story — value is being left on the table here". But if the hand ends with a river check-RAISE/jam
  (hand #80: check-call/check/check → all-in over his bet, all 7 decisions ok), the under-representation
  was the BAIT — the grader already recognises that (line = ok in the plan), only the narrative text does not. Fix idea:
  if the LAST hero action is raise/allin AFTER a passive line and the hand is strong → "trap snapped shut: your
  quiet line invited him to bluff" instead of "value left on the table". Purely a templates/range_story matter;
  the HU GTO bot (Prince) stays untouched — its seesaw/slowplay mixing is already measured and shipped.
...
UNSOLVED spots, measured the brain's river over-sizing, and wrote the forward plan `plans/ROADMAP.md`. Deferred / open:
...
- **[PROPOSED — a new product direction] the personal Claude coaching path** (`plans/ROADMAP.md` §B). Review a
  player's OWN hand histories, engine-grounded + GTO-anchored, personalized to that player's tracked leaks. Most
  machinery exists (`pokerbot/coach/coach.py`, `research/study_grade.py` reconstruction, `research/llm.py`, the new
  `understanding.py`); the one genuinely new piece is a **hand-history parser** for the source site. Phased MVP = a
  review CLI (PokerStars format first). Guardrails: Claude = PC-hub only, hands stay local, cite the engine's exact
  numbers (no hollow praise), and per-session bb/100 is noise — the value is per-decision grading + the cross-session leak trend.
...
## ★ Math-formula fixes + stale-duplicate cleanup (2026-06-20 polish)
...
## ★ LLM curriculum + frac_bad fix (2026-06-17) — completion-only masking + ordered staged shards (`plans/llm_curriculum.md`)
...
(scoped in `plans/NEXT_RUN_TODO.md`, vetted vs 4 sources incl. Supremus + the AAAI-26 DCFR+ paper):
...
## 'Big RAM CFR' = Johanson thesis 2007 — RAM-lever map (agent-read 2026-07-05; real machine: 15.7GB)
- BUILT: **ISO cache** (POKERB_ISO_CACHE, default OFF) — suit-canonical solve-cache key + hole map at
  lookup; PROVEN end-to-end (permuted board -> cache collapse 5.0s->0.0s, identical strategy). Up to 24x
  hit rate on the 12GB cache -> fewer live floor fallbacks. Exact, because ranges are CLASS-level.
- QUEUE #1 (the big one): **canonical FLOP solve LIBRARY** (GS2 precedent: 135k pre-solved abstractions)
  — precomputation bypasses the doubly confirmed flop live NO-GO completely; ~200 top flops x census lines as a
  background CPU job (1-2 days) or pod; targets the flop bleed −6.4…−8.3. POKERB_RSV_FLOP, full ladder.
- QUEUE #2: 7-card LUT evaluator (~124MB RAM, behaviour-preserving) + EXACT turn enumeration instead of MC
  (quality: removes the ~0.9pp MC-SE from borderline decisions; changes decisions -> its own arm).
- CORRECTION of a stale memory: river equity is ALREADY enumerated exactly (equity.py:49-102) — the old
  'river should enumerate' note item has LONG been implemented.
- RNR (ch. 5) = the formalisation of our north-star 'bounded exploit overlay' (Theorem 6: best response
  under an exploitability budget); via DBR-2009 (p per node by observation confidence = our tracker.confidence).
  Deliberately DEPRIORITISED vs GTOW (harvest ~0, dossier) — build it when pools/humans are the mission.
- Mr. Orange trick (ch. 7): equilibrium in the PERTURBED game (+7% winner utility) = aggressive but only
  35mb/g exploitable — candidate for the river-aggression leak, needs its own mini-CFR (numpy, lever E).
- Local DIVAT for non-GTOW measurements (Slumbot/coaching) noted; UCB1 team coach only relevant vs non-stationary
  opponents. Full analysis: agent report (task a33db0cea418f96e9).

## 'Compact CFR' (Eric Jackson, AAAI workshop 2016 — read inline 2026-07-05): the RAM axis from BELOW
Core: 16 bytes/action -> **1 byte** via (1) follow-the-leader instead of regret matching (only the argmax counts ->
regrets as non-negative OFFSETS from the best), (2) 1-byte quantisation (fixed-size for external-sampling
random access; costs some convergence time), (3) current-strategy-only instead of average (FTL -> automatically PURE
= 1 bit/action), (4) checkpoint MIXTURES partially rescue the exploitability (51 blends: 21.7 vs 10.14 mbb/g).
HONEST warnings from the paper itself: FTL loses the no-regret GUARANTEE; current-only = poor
exploitability (only head-to-head ok) — for our anti-GTOW mission exploitability is the yardstick, so
keep average/mixtures; the OFFSET+1-byte regrets are the safe part.
**THE RAM-ADAPTIVITY DIAL (user wish, design principle):** Johanson 2007 scales quality with RAM UPWARD
(more resolution = stronger), Jackson 2016 DOWNWARD (16x compression) — one core, one dial:
float32 regrets (lots of RAM) -> uint8 offsets -> purified 1-bit (little RAM). Installation sites:
- **flop_library (queue #1):** dumps as uint8-quantised strategies (256 levels << solver noise) instead of
  JSON floats -> ~8-20x smaller -> the 1755-flop library fits into the real 15.7GB RAM.
- **v4 CFR core:** regret tables with selectable precision (the dial as a constructor parameter).
- Optional: the 12GB solve cache as npz/uint8 for RAM preload (disk is cheap — only if needed).
No live benefit TODAY (TexasSolver subgames are time-bound, not RAM-bound). PDF: books/papers/CFR/.

## Tournament mode (2026-08-04) — deliberate approximations
- **OPEN_FRAC "UTG" find**: "UTG" is missing from the 6-max OPEN_FRAC (falls back to 0.20 instead of EP 0.16). DELIBERATELY
  not fixed (anchor protection of the measured tag core); gated re-test arm when the core is re-measured.
- **Dead-button rule**: the tournament director simply lets the button move on to the next survivor
  (instead of dead button/dead SB). Exact later, if empirical comparison demands it.
- **ICM = Malmuth-Harville**: no future-game-simulation corrective (blind positions). Adjustable model knob.
- **BF with start-of-hand stacks**: the bubble-factor approximation uses the stacks at the start of the hand (stable, cacheable);
  only the exact all-in call computes with the behind stacks at the moment of decision.

## Entropy estimator: the null control is mandatory (2026-08-16)

In an entropy measurement over finely resolved classes, the null control (class labels
permuted so that by construction the channel carries nothing) yielded, instead of the expected H₀ = 1.780, a
value of **1.897 — bias +0.116 bit, larger than the entire measured effect (0.091)**. At
coarser resolution of the same measurement the same bias was +0.001 and +0.008 respectively.

**Rule from this:** The plug-in estimator is biased upward with many classes and little data,
and most strongly exactly where the resolution is finest — i.e. where one suspects the
most interesting effect. Every entropy/information statement in this repo needs the
permuted null control NEXT TO the measured value, otherwise it is unreadable. (Open: switch to a
bias-corrected estimator, e.g. Miller-Madow or NSB.)

## GPU TurnCFR: IP frequency difference to TexasSolver (2026-08-30, open)
Turn cross-validation (research/gpu_vs_texassolver_turn.py, one-arm collapse eff=0.75*Pot):
OOP bet delta 0.031, OOP call delta 0.000 (exact), per-combo OOP correlation 0.944 —
but IP bet-after-check 0.416 (TurnCFR, expl 0.002%Pot, stable over 100..1600 iters)
vs 0.321 (TexasSolver, stable over accuracy 0.1..0.02). Both highly converged.
SUSPICION: equilibrium-selection difference (2p zero-sum: value unique, frequencies
at indifferent nodes not). NOT PROVEN — a subtle tree difference is possible.
Exact discriminator (if needed): load the TS strategy into the TurnCFR tree and measure its
exploitability in OUR game (~0 => multiplicity confirmed; large => tree bug).
The river stage is unaffected (correlation 0.999, all deltas <0.007).

## EXPLAINED (2026-09-01): TurnCFR IP frequency difference = solver-dependent equilibrium selection
The open question of 2026-08-30 (TurnCFR 0.416 vs TexasSolver 0.321 with both expl~0) is theoretically
placed by Leal, "Which Nash Equilibrium? Solver-Dependent Selection on Zero-Sum Nash Polytopes"
(arXiv 2606.28308, books/papers/CFR/): 2p0s games have Nash POLYTOPES
(same value, materially different behaviour); WHICH member a solver picks is a
function of the ALGORITHM (not the seed), and regret-averaging families (CFR/CFR+) drift to
low-entropy faces (94% of the 180 asymmetric games, p<1e-27), while regularised
last-iterate methods (R-NaD/MMD) hit the max-entropy member (100%). Both our solvers are
CFR-family in different variants -> different polytope members. NO MORE tree-bug suspicion.
FOLLOW-UP FINDING (Kuhn, Fig. 8): the max-entropy member WEAKLY dominates the CFR+ member against ALL
faulty opponents (25/25) with an exact tie on-path — and this hedge advantage is an
EXTENSIVE-FORM phenomenon (x5.6 vs matrix games). Build candidate from this: entropy anchor in the GPU CFR
(MMD-like), gates: expl equal + entropy higher + adversary harvest (exploit_jagd/Fable) drops.
