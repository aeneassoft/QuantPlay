...
> **(A) make the bot understand poker as well as possible**, and **(B) build a Claude-API coaching path for the
> user personally**. Honest throughout: MEASURED vs PROPOSED is marked explicitly. Last updated **2026-06-29**.
...
## §B — The personal Claude coaching path (for the user)

> **Goal:** Claude reviews the USER's OWN real hands (CoinPoker / PokerStars), grounded in our engine (exact
> equity/solver/blueprint + the new understanding layer), and produces honest, personalized, GTO-anchored coaching —
> tuned to the user's known tendencies. This is a NEW product direction; most of the machinery already exists.
...
| The user's known leaks (personalization) | the cross-session memory `user-poker-style` (loose blinds OOP, loose calling, variance) |

### B1 — The architecture (per-session, then cross-session)
1. **Ingest** the user's hands: a hand-history export (PokerStars format is already in the loop; a **CoinPoker
   adapter** is the one new parser — map its HH to the same internal state the reconstruction seam expects).
...
4. **Coach with Claude:** feed `format_spot(spot)` + the strategic read + the oracle mix + the user's action to
   ...
   lesson* — phrased in the user's two-gear (exploit/GTO) style, in German.
...
6. **Personalize across sessions:** track the user's recurring leaks over time (seed from `user-poker-style`; update it
   as new sessions confirm/retire a leak) so coaching focuses on THE user's patterns, not generic advice.

### B2 — Phased build (smallest valuable first)
- **B2.1 — MVP ($0 infra, a few $ Claude):** a CLI `pokerbot/coach/review_session.py` that takes ONE PokerStars HH
  file → reconstructs → grounds → Claude per-decision review → a Markdown report. Reuses `study_grade` + `coach.py` +
  `understanding.py`; no new parser (PokerStars first). Verify on one of the user's exported sessions.
- **B2.2 — CoinPoker adapter:** add the CoinPoker HH parser (the only genuinely new code) so the user's primary site
  works end-to-end.
- **B2.3 — Personalization + trend tracking:** persist per-leak EV-loss across sessions → a "your leaks this week"
  rollup; feed the tracked leaks back into the coach prompt.
- **B2.4 — A nicer surface (optional):** a small local web view (mirror `pokerbot/web`) for browsing graded hands +
  the coaching notes, instead of a Markdown file.

### B3 — Honest scope / guardrails
- **Cost + privacy:** Claude API = PC-hub only (CLAUDE.md hard rule); the user's hands stay local; keys read from
...
- **It is coaching, not autopilot:** this reviews the user's OWN play to make the USER better; it is separate from the
