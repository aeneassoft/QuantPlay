...
design → adversarial verify; verifier caught a cross-phase six.html file conflict, duplicate glossary systems,
a phantom-record logger bug, enum drift, and the missing mode toggle — all fixed as ★-BINDING directives in
the plan). Verdict: one-run executable = TRUE. 26 work packages P0-0…P4-B in verified dependency order;
six.html stays byte-identical (training.html is the trainer UI); the PRINCE bot serves as the GRADING ORACLE
(exploit stays OFF for grading in both modes). NEXT: the user runs the plan in an ultracode session.
