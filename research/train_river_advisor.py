"""WS2: train the RIVER advisor (predict solver P_bet on the river from blocker/made/texture features) and
compare to the FREQUENCY-ONLY baseline (per role x texture mean). Held out BY BOARD (5-card) to test
generalization. Mirrors train_turn_advisor.py. Saves knowledge_base/postflop/river_advisor.pt only if the
feature model beats the table by >3% (else the river stays on the #40 heuristic).
Run: python -m extraction.train_river_advisor
"""
...
def main() -> None:
    rows = [json.loads(x) for x in (config.DATA_DIR / "river_data.jsonl").read_text(encoding="utf-8").splitlines() if x.strip()]
    rng = random.Random(7)
    if len(rows) > CAP:
    ...
    print(f"held-out-by-board RIVER P_bet MSE:  freq-baseline {base_mse:.4f}  |  MLP {mlp_mse:.4f}  "
    ...
    if mlp_mse < base_mse * 0.97:
        ...
        torch.save({"state": net.state_dict(), "dims": Xtr_t.shape[1]},
                   config.KNOWLEDGE_DIR / "postflop" / "river_advisor.pt")
        print("MLP beats the baseline -> saved river_advisor.pt (wire into bot.py river branch)")
    else:
        print("MLP does NOT clearly beat the baseline -> ship the river freq table, skip the net")
